...
class PDMGenerator:
    ...
    def _initialize_states(self, lateral_batch_idcs: List[int]) -> None:
        ...
        self._state_array[lateral_batch_idcs, 0, StateIndex.VELOCITY_2D] = self._initial_ego_state.dynamic_car_state.rear_axle_velocity_2d._array
        self._state_array[lateral_batch_idcs, 0, StateIndex.ACCELERATION_2D] = self._initial_ego_state.dynamic_car_state.rear_axle_acceleration_2d._array
        self._state_array[lateral_batch_idcs, 0, StateIndex.STEERING_ANGLE] = self._initial_ego_state._tire_steering_angle
        self._state_array[lateral_batch_idcs, 0, StateIndex.STEERING_RATE] = self._initial_ego_state.dynamic_car_state.tire_steering_rate
        self._state_array[lateral_batch_idcs, 0, StateIndex.ANGULAR_VELOCITY] = self._initial_ego_state.dynamic_car_state.angular_velocity
        self._state_array[lateral_batch_idcs, 0, StateIndex.ANGULAR_ACCELERATION] = self._initial_ego_state.dynamic_car_state.angular_acceleration
    ...
    def _update_leading_agents(
        ...
        if not update_leading_agent:
            ...
            for proposal_idx in lateral_batch_idcs:
                ...
                if len(agents_ahead) > 0 and len(lateral_batch_idcs)!=1:  # red light, object or agent ahead
