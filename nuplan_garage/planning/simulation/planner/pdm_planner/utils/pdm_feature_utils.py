...
def create_pdm_feature(
    ...
    if isinstance(centerline, PDMPath):
        ...
        centerline_progress_values = (
            ...
            * model.centerline_interval
            + current_progress
        )  # distance values to interpolate [120,], >62.86
        ...
        )  # convert to relative coords [120,3], np.ndarray
    else:
        planner_centerline = convert_absolute_to_relative_se2_array(
            current_ego_state.rear_axle, centerline,)  # convert to relative coords
    ...
    if closed_loop_trajectory is not None:
        ...
        sample_idx = np.arange(0,planner_centerline.shape[0],planner_centerline.shape[0]//model.trajectory_sampling.num_poses)[:model.trajectory_sampling.num_poses]
        planner_trajectory = planner_centerline[sample_idx]
