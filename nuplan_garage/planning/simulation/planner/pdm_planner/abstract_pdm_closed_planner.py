...
from nuplan_garage.planning.simulation.planner.pdm_planner.proposal.pdm_generator import (
    PDMGenerator,
)
...
class AbstractPDMClosedPlanner(AbstractPDMPlanner):
    ...
    def __init__(
        ...
        self._generator = PDMGenerator(trajectory_sampling, proposal_sampling, leading_agent_update_rate)
    ...
    def _update_proposal_manager(self, ego_state: EgoState, better_anchor):
        """
        ...
        if create_new_proposals:
            if better_anchor:
                proposal_paths: List[PDMPath] = self._get_proposal_paths(better_anchor)
            else:
    ...
    def _get_proposal_paths(
        ...
        """
        if isinstance(current_lane, LaneGraphEdgeMapObject):
            centerline_discrete_path = self._get_discrete_centerline(current_lane)
        else:
            centerline_discrete_path = current_lane
    ...
    def _get_closed_loop_trajectory(
        ...
        self._update_proposal_manager(ego_state, better_anchor=better_anchor)
        ...
        if not self._lateral_offsets:
            # retrun free trajactory
