...
class LatticePlanner: 
    ...
    def calculate_cost(self, path, obstacles, vehicles):
        # path curvature
        curvature = self.calculate_path_curvature(path[0][0:100])
        ...
        target = self.check_target_lane(path[0][0:50:10], path[3], vehicles)

        # check obstacles
        obstacles = self.check_obstacles(path[0][0:100:10], obstacles)

        # out of boundary
        out_boundary = 0 #self.check_out_boundary(path[0][:100], path[2])
